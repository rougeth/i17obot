...
import config
from templates import get_template_for_user
...
async def start(message: types.Message):
    ...
    response = await get_template_for_user(message.from_user.id, "start")
    await bot.send_message(
        message.chat.id,
        response.format(name=message.from_user.first_name),
...
async def reminder(message: types.Message):
    ...
    response = await get_template_for_user(message.from_user.id, template_name)
...
async def translate_at_transifex(message: types.Message):
    ...
    response = (
        await get_template_for_user(message.from_user.id, "translate_at_transifex")
    ).format(
...
async def status(message: types.Message):
    ...
    response = (
        await get_template_for_user(message.from_user.id, "status")
    ).format(
        users=len(users),
...
async def links(message: types.Message):
    response = await get_template_for_user(message.from_user.id, "links")
    await bot.send_message(
...
async def tutorial_1(message: types.Message):
    if isinstance(message, types.CallbackQuery):
        response = await get_template_for_user(message.from_user.id, "tutorial_part_1")
...
async def tutorial_2(query: types.CallbackQuery):
    ...
    response = await get_template_for_user(query.from_user.id, "tutorial_part_2")
...
async def tutorial_3(query: types.CallbackQuery):
    ...
    response = await get_template_for_user(query.from_user.id, "tutorial_part_3")
    await tutorial_callback_query(
        query,
        message=response,
